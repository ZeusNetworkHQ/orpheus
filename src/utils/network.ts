...
const NETWORK_CONFIG_MAP: NetworkConfigMap = {
  ...
  [`${BitcoinNetwork.Testnet}-${SolanaNetwork.Devnet}`]: {
    binanceUrl: "https://www.binance.com/api",
    aresUrl: process.env.NEXT_PUBLIC_TESTNET_DEVNET_ARES_URL ?? "",
    aegleUrl: process.env.NEXT_PUBLIC_TESTNET_DEVNET_AEGLE_URL ?? "",
    hermesUrl: process.env.NEXT_PUBLIC_TESTNET_DEVNET_HERMES_URL ?? "",
    solanaUrl:
      process.env.NEXT_PUBLIC_SOLANA_DEVNET_RPC ??
      "https://api.devnet.solana.com",
    customSolanaUrl: getLocalStorage(CUSTOM_SOLANA_DEVNET_RPC_KEY) ?? "",
    bitcoinExplorerUrl:
      process.env.NEXT_PUBLIC_TESTNET_BITCOIN_EXPLORER_URL ??
      "https://bitcoin-regtest-devnet.zeusscan.work/",
    bootstrapperProgramId:
      process.env.NEXT_PUBLIC_DEVNET_BOOSTRAPPER_PROGRAM_ID ?? "",
    liquidityManagementProgramId:
    ...
    assetMint: process.env.NEXT_PUBLIC_TESTNET_ASSET_MINT ?? "",
  },
};
